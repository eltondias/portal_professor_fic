...
window.onload = function() {
   ...
   document.getElementById("carregando").className="oculta";
            ...
            <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
            <title>Caderneta Impressa</title>
    </head>
    <link href="../css/lacto_notas_imp.css" rel="stylesheet" type="text/css">
...
<h3>
 		...
 		where ano='" . $_SESSION['ano'] . "' and seqano='" . $_SESSION['seqano'] . "' and ava='" . $_SESSION['avaliacao'] . "'";
										...
										echo'Mapa de avaliacao - Turma - Ìtens </br>' ; 
                                        echo "Lançamento de notas : " . utf8_encode($_SESSION['periodo']) . " -  Turma: " . utf8_encode($_SESSION['turma']) . " - Avaliação:" . utf8_encode($row_item_ava['des']) . "   -  Disciplina:" . utf8_encode($_SESSION['disciplina_des']). "&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;"; echo utf8_encode($_SESSION['nome']) . "-" . utf8_encode($_SESSION['matric']);
                                        ?></h3>
                                        ...
                                        <table class='tabelas' id="tabela_aluno" width="100%" >
                                            ...
                                            for ($i = 1; $i <= 20; $i++) {
                                                ...
                                                if (!($row_item[$item_nota_max])<>'null') {                      
															...
															<label style='display:none' width='50' readonly class='cabecalho'   "
															. "id='$id_max' value='" . str_pad($row_item[$item_nota_max], 3, ".0", STR_PAD_RIGHT) . "'                                              					></label>". "   
                                                    ...
                                                    for ($i = 1; $i <= 20; $i++) {
                                                        ...
                                                        if (!($row_item[$item_nota_aluno])<>'null'){
                                                            ...
                                                            if (is_numeric($ROW_NOTAS[$item_nota_aluno])) {
                                                                ...
                                                                echo '<td><label width="20" value=' . $ROW_NOTAS[$item_nota_aluno] . 
																'  class="notas" name="' . utf8_encode($row_item['MATRIC']) . '-' . $item_nota_aluno . 
																'" id="' . utf8_encode($row_item['MATRIC']) . '_' . $coluna_nota . 
																'" >'. $ROW_NOTAS[$item_nota_aluno] .'</label></td>';
                                                            } else {
                                                                echo'<td><label width="20" value="" name=' 
																. utf8_encode($row_item['MATRIC']) . '-' . $item_nota_aluno . ' class="notas"  id="' 
																. utf8_encode($row_item['MATRIC']) . '_' . $coluna_nota . '" >'. $ROW_NOTAS[$item_nota_aluno] .'</label></td>';
                                                            }
                                                        }
                                                    }

                                                   
                                                        echo"<td><label readonly class='notas' value='" . $nota_total . "'>". $nota_total ." </label></td>";
...
//                                            }
			  ...
			  <input type='button' id='bt_imprimir' onclick='window.print()' value='Imprimir' />";*/
                                            ?>
                                            </div>
                                            </div>
                                            <div id="clear"></div>
