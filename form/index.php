...
<body>
    ...
    <form method="post" id="formulario_contato" onsubmit="validaForm(); return false;" class="form" action="index.php">
		...
		<p class="name">
          ...
          <input  name="cpf" id="cpf" placeholder="CPF sem ponto nem traço" />
		</p>
...
//verifica se o professor está ativo
	...
	if($fet_logar['cpf'] <> $_POST['cpf']  ){
		echo"<script> alert('CPF incorreto tente novamente!');</script>";
 ...
 <script type="text/javascript">
	    ...
	    function validaForm()
        {
           ...
           /* if($('#nome').val() == '')
			...
			if($('#palavra').val() == '' && !erro) {
                alert('Você precisa preencher as letras informadas');erro = true;
