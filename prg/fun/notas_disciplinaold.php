...
class Sec_prof_lacto{
	...
	final function avaliacao(){
		
		$sql_item = "select  ava from sec_avaprof
 			where ano=".$_SESSION['ano']." and seqano=".$_SESSION['SEQANO']." AND not GETDATE() <= perini and not GETDATE() >= perfin ";
		...
		$retorno.='<option value="'.$registro['ava'].'" disabled="disabled" selected="selected">Escolha a avaliação </option>';
