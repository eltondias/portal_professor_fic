...
class Sec_prof_discip{
	...
	function __construct(){
			...
			$this->avaliacao = trim(addslashes(strip_tags(htmlspecialchars($_POST['disciplina']))));
			$_SESSION['disciplina'] = $this->avaliacao;
			$this->avaliacao_professor();
			}
			...
			if(isset($_POST['avaliacao'])){				
				...
				$sql="select  ava,des from sec_avaprof
 		where ano='".$_SESSION['ano']."' and seqano='".$_SESSION['seqano']."' AND not GETDATE() <= perini and not GETDATE() >= perfin order by des asc ";
		
				$cur_avaliacao_professor = mssql_query($sql);
				while($row_avaliacao_professor  = mssql_fetch_array($cur_avaliacao_professor )){
					
				$retorno .= "<option value='".$row_avaliacao_professor ['ava']."'>".trim(utf8_encode($row_avaliacao_professor ['des']))."</option> ";
	...
	final function avaliacao_professor(){
		
		$sql_ava="select  ava,des from sec_avaprof
 		where ano='".$_SESSION['ano']."' and seqano='".$_SESSION['seqano']."' AND not GETDATE() <= perini and not GETDATE() >= perfin order by des asc ";

		$cur_avaliacao_professorold  = mssql_query($sql_ava);
		
		
		$row_avaliacao_professorold = mssql_fetch_array($cur_avaliacao_professorold);
        if (!empty($row_avaliacao_professorold ['ava'])) {
            $sql="select  ava,des from sec_avaprof
 		where ano='".$_SESSION['ano']."' and seqano='".$_SESSION['seqano']."' AND not GETDATE() <= perini and not GETDATE() >= perfin order by des asc ";

		$cur_avaliacao_professor  = mssql_query($sql);
            $retorno = "<option disabled='disabled' selected='selected' value='' >Escolha uma Avaliação</option>";
             
            
            while ($row_avaliacao_professor = mssql_fetch_assoc($cur_avaliacao_professor)) {

                $retorno .= "<option value='" . $row_avaliacao_professor ['ava'] . "'>" . utf8_encode($row_avaliacao_professor ['des']) . "</option> ";
            }
        }
        else{
            $retorno = "<option disabled='disabled' selected='selected' value='' >Período Encerrado</option>";
        }
